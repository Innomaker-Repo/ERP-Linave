...
export function PropostaView() {
  ...
  const handleRecusaCliente = (obra: any) => {
    const confirmacao = window.confirm('Tem certeza que deseja recusar esta proposta? Uma nova versão será criada automaticamente.');
    ...
    const propostasAtualizadas = obra.propostas.map((p: any, idx: number) => 
      idx === obra.propostas.length - 1 ? { ...p, status: 'recusada' as const } : p
    );

    // Criar nova versão copiando o conteúdo
    const novaVersao = propostasAtualizadas.length + 1;
    const novaProposta = {
      versao: novaVersao,
      dataCriacao: new Date().toISOString().split('T')[0],
      status: 'pendente' as const,
      dataProposta: ultimaProposta.dataProposta,
      numeroProposta: `LN-0${novaVersao}/26`,
      cliente: ultimaProposta.cliente,
      atribuidoA: ultimaProposta.atribuidoA,
      cargoContato: ultimaProposta.cargoContato,
      referencia: ultimaProposta.referencia,
      saudacao: ultimaProposta.saudacao,
      ...
      preco: ultimaProposta.preco,
      impostos: ultimaProposta.impostos,
    ...
    const obraAtualizada = {
      ...obra,
      propostas: [...propostasAtualizadas, novaProposta]
    };

    const obrasAtualizadas = obras?.map((o: any) => o.id === obra.id ? obraAtualizada : o) || [];
    saveEntity('obras', obrasAtualizadas);

    alert('Proposta recusada. Nova versão criada automaticamente!');
  };
          ...
          <>
            <div className="border-t border-white/10 pt-8">
              ...
              <div className="space-y-4">
                {negociosNegociacao.filter((obra: any) => !obra.propostas || obra.propostas.length === 0).map((obra: any) => (
                  <div key={obra.id} className={sectionClass}>
                    ...
                    >
                      <FileText size={16} /> Fazer Proposta
                    </button>
                  </div>
                ))}
              </div>
    ...
    <div className="p-8 space-y-6 animate-in fade-in duration-500">
      ...
      <div className={sectionClass}>
        ...
        <div className="space-y-1.5">
          ...
          <textarea 
            className={`${inputClass} h-32`}
            value={propostaForm.escopoA}
            onChange={e => setPropostaForm({...propostaForm, escopoA: e.target.value})}
          />
        </div>
      </div>
